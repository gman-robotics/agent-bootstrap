...
Launch all reviewers in a single message using the harness subagent tool (`Task()` in Claude Code / Cursor). Use the `interrogate reviewers` list from project model-routing rules (or `skills/subagent-routing/SKILL.md` defaults) when present, one reviewer per entry, extending or shrinking the Reviewer A/B/C/D labels below to the configured entry count. Otherwise use the table defaults.

| Subagent | Default model |
|----------|---------------|
| Reviewer A | Sonnet-tier model (see skills/subagent-routing/SKILL.md) |
| Reviewer B | Sonnet-tier model (see skills/subagent-routing/SKILL.md) |
| Reviewer C | Haiku-tier model (see skills/subagent-routing/SKILL.md) |
| Reviewer D | Sonnet-tier model (see skills/subagent-routing/SKILL.md) |

For each reviewer:
- `subagent_type`: per `skills/subagent-routing/SKILL.md`
- `model`: the configured `interrogate reviewers` entry, or the table default with no configured line
- read-only subagent mode when the harness supports it
