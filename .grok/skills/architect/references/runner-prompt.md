...
- Shared state: if two actors might both write, ask "what happens?" If the answer isn't "nothing," default to per-actor state with a merge at the read boundary, per the **pstack-principles** (separate-before-serializing-shared-state) principle.
...
- Encode invariants in types: hard-to-misuse types > runtime checks > prose comments, per the **pstack-principles** (encode-lessons-in-structure) principle.
- Validate at boundaries, trust types inside, per the **pstack-principles** (boundary-discipline) principle. Business logic as pure functions. The shell stays thin.
- Single source of truth per invariant. Derive instead of sync.
- Idempotent state transitions where applicable, per the **pstack-principles** (make-operations-idempotent) principle. Ask what happens if the operation runs twice or crashes halfway.
- Short call chains. If tracing the flow needs more than three files, flatten the hierarchy, per **pstack-principles** (laziness-protocol) and **pstack-principles** (minimize-reader-load).
