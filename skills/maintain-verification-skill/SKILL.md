...
description: >-
  Periodic pass that keeps a project's verification skill and feature map
  honest. Use for "audit the verify skill".
version: 1.0.0
---
...
**Purpose**
Periodic pass keeping a project's verification skill and feature map honest: parallel source readers per feature, one live session driving every feature.

**Trigger**
"audit the verify skill" or scheduled upkeep of a `create-verification-skill` output.

**Do not use for**
- No verification skill exists yet → `create-verification-skill`
- Product code fixes → report regressions; do not edit product code in this pass

## Companions

| Skill | Role here |
|---|---|
| `create-verification-skill` | Generator for the skill under maintenance |
| `pstack-principles` | prove-it-works — live pass required even when source looks clean |

**Verification**
- Outcome declared: clean / changed / blocked
- Every feature file got source wave + live drive (or verified-unreachable with prerequisite)
- At most one PR of proven corrections to the verify skill directory
...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `skills/verify-*/`). Several candidates → ask which one; none → stop and point at `create-verification-skill` instead of inventing a target.
