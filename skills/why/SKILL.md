...
- `subagent_type`: per `skills/subagent-routing/SKILL.md`
- `model`: your configured why-investigators model (default Haiku-tier model (see skills/subagent-routing/SKILL.md))
- writable subagent mode when MCP or spot-check access is required (agent mode). **Do not use readonly/Ask mode.** It strips MCP access, which disables MCP-backed investigators entirely. Investigators still shouldn't write anything.
...
- `subagent_type`: per `skills/subagent-routing/SKILL.md`
- `model`: your configured why-synthesizer model (default Sonnet-tier model (see skills/subagent-routing/SKILL.md))
- writable subagent mode when MCP or spot-check access is required (agent mode). The synthesizer's quality check spot-verifies citations, which can require MCP access. Readonly/Ask mode strips MCPs and defeats that.
