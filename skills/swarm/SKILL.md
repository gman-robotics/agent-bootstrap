...
4. Pick the worker model from `swarm workers` in project model-routing rules (or `skills/subagent-routing/SKILL.md` defaults) when present. Otherwise use Haiku-tier model (see skills/subagent-routing/SKILL.md). For a model race, name each arm's model up front.
