...
**Task "Bootstrap leftover gates"** (2026-08-26, cloud agent, branch `cursor/bootstrap-leftover-gates-6517`). Follow-up to merged [PR #11](https://github.com/gman-robotics/agent-bootstrap/pull/11) (`main` @ `377cfd8`) — a pass-3 review found three should-fix leftovers, distinct from #11's own pass-2 "INDEX bind" blocker. New branch, new PR; #11 itself not reopened, not merged. See `memory-bank/progress.md` 2026-08-26 "Bootstrap leftover gates" entry for the full breakdown.
- **Leftover 1**: `GRANDFATHERED_SKILLS` was already a `frozenset` of the original 20 names as of `377cfd8` — nothing to fix in the value itself. Added `test_grandfathered_skills_is_frozen_at_the_original_twenty` to pin the type + count so any future growth requires a visible test edit.
- **Leftover 2**: `python3 scripts/index_skills.py` raised `ModuleNotFoundError` with no `PYTHONPATH` (script dir, not repo root, lands on `sys.path[0]`). Fixed with a guarded `sys.path` insert before the `scripts.check_skill_live` import; added a real-subprocess regression test with `PYTHONPATH` stripped.
- **Leftover 3**: `CONTRIBUTING.md` §1 and `skills/close-out/SKILL.md` Step 9.4 implied the mechanical test enforces AGENTS.md/trigger-table placement too; it only parses `skills/INDEX.md`. The five trigger-list files use three different formats — not a clean parser extension — so took the smaller honest fix: reworded both (+ regenerated the `.grok/skills/close-out` mirror via the real exporter) to scope the claim to `skills/INDEX.md`. `close-out` bumped `1.3.0` → `1.3.1`, black-box run record recaptured, hub version `0.9.0` → `0.9.1`.
- Verification: `python3 -m unittest discover -s tests` — 26/26 pass. `python3 scripts/check_skill_live.py close-out` exits `0`.
