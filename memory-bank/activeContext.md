...
**Task "hoh-schema-steal" — IMPLEMENT-TRACK** (2026-09-02, cloud agent, new branch `cursor/implement-hoh-schema-steal-4b7c`, new draft PR against `main` at `08831ab` — plan PR [#15](https://github.com/gman-robotics/agent-bootstrap/pull/15) squash-merged). Ships GB-1..6 and H-1..5 as real, live-gated skills per the approved plan (`docs/projects/agent-bootstrap/hoh-schema-steal-plan.md`) — the plan document itself is **not reopened**, only 9 off-by-one section cross-references were fixed (§ numbers pointed at §8 when the target was §7/§9/§10/§14; the wiki-citation §2/§15 content and its mechanical test were left untouched, confirmed green throughout).

**What shipped**:
- `skills/evidence-packet-protocol/SKILL.md` + `SCHEMA.md` (new, `1.0.0`): the `E_t.json` evidence packet — `head_sha` required (GB-4), `qa_status`/record `status` restricted to `verified | gap` only at both levels including the crossed-pair case (GB-1/GB-6), non-empty typed `execution_records` (`screenshot | runtime_trace | fixture`, GB-1), `planner_handoff` structural non-emptiness rules (GB-3), forbidden living-PII scan (GB-1 extension), `evidence/E_t.json`/`evidence/E_<n>.json` path convention (H-1), role write-permissions (H-2), warm-start contract quote (H-3), per-role commit convention (H-4), progressive-disclosure index pointer (H-5).
- `skills/preservation-gate/SKILL.md` (new, `1.0.0`): the exact `## Preservation Gate` heading, its comparison table vs. REPEAT (positive/never-closes vs. negative/mechanically-closed), and the placement decision (own skill, not a `reply-contract` patch — `reply-contract` untouched).
- Four real validator scripts (TDD, red-then-green confirmed for every one before implementation existed): `scripts/validate_evidence_packet.py`, `scripts/validate_preservation_gate.py`, `scripts/check_planner_reads_et.py`, `scripts/check_evidence_index_is_progressive.py`.
- Nine named black-box fixtures (`case.json` + `README.md` each, real sample packets/markdown, no `<placeholder>` argv), with the Blair-pass-2-style should-fixes folded in directly rather than shipped as known gaps: `et-schema-valid`, `et-status-not-verified-or-gap` (crossed enum pair — **two** sample packets, `E_t.partial-blocked.json` + `E_t.blocked-partial.json`, asserting all four tokens in one run), `et-missing-execution-record`, `et-living-pii`, `freeze-sha-mismatch` (literal 40-char `aaaa...`/`bbbb...` SHAs), `schema-retry-then-escalate` (exit `1` + `ESCALATE`, never exit `2` — that stays reserved for the runner's own environment-blocked verdict, documented explicitly in `SCHEMA.md`), `next-planner-reads-et`, `evidence-index-not-full-dump`, `dt-missing-preservation-gate`.
- `tests/test_evidence_packet_protocol_fixtures.py` — the `attack-12-fixture-io` mechanical class itself: actually runs every fixture's literal argv via `scripts/run_black_box_fixture.py`'s own `load_case`/`run_case` and asserts a real `pass` verdict, plus a placeholder-free-argv check and a README-names-the-class check. Every fixture README + both new `SKILL.md` Common Mistakes tables name this class explicitly, per the task's Attack 12 instruction.
- `skills/multi-harness-coordination/SKILL.md` (`1.0.0` → `1.1.0`): one new "Optional: Preservation Gate field" subsection immediately after the existing four-field-envelope subsection — pointer only, no mechanism copy, no rewrite of the rest of the skill. `9259d42`'s envelope content untouched. `skills/reply-contract/SKILL.md`, `skills/plan-code-review-workflow/SKILL.md`, `skills/expert-pr-review/SKILL.md` all untouched; no `skills/adversarial-review/` created.
- Gated correctly: captured real `skills/evidence-packet-protocol/black-box-run.json` and `skills/preservation-gate/black-box-run.json` via `scripts/run_black_box_fixture.py` **before** adding either to `skills/INDEX.md`; `scripts/check_skill_live.py` exits `0` for both; `scripts/index_skills.py` confirms every non-grandfathered INDEX.md entry is live-eligible (neither new skill added to `GRANDFATHERED_SKILLS`). Editing `evidence-packet-protocol/SKILL.md` a second time (to add its Leftovers section) was directly observed to flip `check_skill_live.py` to `stale` until recaptured — the same silent-refine guard this hub already relies on, working as designed on its own new maintainer.
- `scripts/export_codex_skills.py`: two new `SkillConfig` entries; re-exported `.grok/skills/` (`--force`) — confirmed only `multi-harness-coordination`'s and `evidence-packet-protocol`'s `references/source.md` changed beyond the two new skill folders, and `grill-with-docs`'s hand-added reference files survived intact.
- `skills/INDEX.md` two new entries (added only after the live-gate pass above), hub version `0.9.3` → `0.10.0` (`AGENTS.md`, `INDEX.md` footers); `AGENTS.md` §4 table + all five per-harness trigger files (`.clinerules`, `.kilocoderules`, `.cursorrules`, `.openhands_instructions`, `.cursor/rules/agent-bootstrap.mdc`) updated with both new skills, matching each file's existing format exactly.
- **Hard do-nots honored**: no clone of `Flesymeb/HarnessOfHarness`, no Hardproof/OpenCode/HoH-runtime install, no GMA-9/14/8/13/AWS reopened or started, no living PII beyond the intentional `Lisa`/`Tanya` forbidden-fixture pair (no real phone numbers anywhere — only the reserved `+1555XXXXXXX` fictitious range appears in any sample), no teammate entity pages minted, no Grok Bot agent UUIDs in hub skills, Sonnet vs. Grok / Claude Code vs. Codex model-pairing distinction preserved in H-2's prose.
- **Named leftovers** (in `skills/evidence-packet-protocol/SKILL.md`'s own "Leftovers" section, mirroring the plan's §14): no HoH runtime/scheduler, Hardproof not installed, native-compiled-black-box gap unresolved, "Eleanor" `reply-contract` `1.3.0` overlay not reconciled, no Grok Bot CoS Lane/Kit overlay wiring in this hub (cannot live here per the plan's hub-vs-overlay split).

**Verification**: `python3 -m unittest discover -s tests` — **96/96 pass** (45 pre-existing + 51 new). Every one of the 9 fixtures was proven red-then-green during TDD (see `git log` on this branch for the exact red/green sequence) and re-confirmed passing for real via `tests/test_evidence_packet_protocol_fixtures.py`'s live subprocess execution, not a description. Branch: `cursor/implement-hoh-schema-steal-4b7c`. New draft PR against `main`; hold merge per instructions — Blair impl-reviews, then Kit black-box-tests (reading the PR is not a pass for Kit either).

## Superseded Focus (plan-track, merged as PR #15 at `08831ab`) — "hoh-schema-steal" plan revision
