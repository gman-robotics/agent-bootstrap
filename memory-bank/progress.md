...
**What Was Done** (plan §7 sequencing, followed strictly)
- [x] **Step 1 — Vendor tree + hub SKILL.md**: Cloned `cloudflare/security-audit-skill` at pinned `c1c8a8c1471069fb0e188eeaff69b8e8db6564a8`, verified the tree matches the plan's listing exactly (13 domain packs, `report-schema.json`, `README.md`, 2 `.cjs` + `.test.cjs` pairs). Vendored verbatim under `skills/security-audit/references/cloudflare/` (dual-home). Authored house `skills/security-audit/SKILL.md` as the hub entry — narrow allowlist `description:`, guidance-mode-by-default, the four named Universal execution-safety controls read verbatim from the vendored upstream `SKILL.md`, `needs_validation` fallback, three explicit non-conflation statements (harness sandbox / `expert-pr-review` build-test / bare assertion vs. this skill's four-control check). House `NOTICE.md` with URL/SHA/date/license.
- [x] **Step 2 — Companions + blast-radius recapture**: Added `("security-audit", "expert-pr-review")` and `("security-audit", "blast-radius")` to `COMPANION_PAIRS` in `scripts/validate_pstack_skill.py` (already-existing mechanism, zero new code). Added bidirectional `Do not use for`/`Companions` rows to `skills/expert-pr-review/SKILL.md` (grandfathered, no live-gate consequence) and `skills/blast-radius/SKILL.md` (live-gated). Confirmed `check_skill_live.py blast-radius` went stale after the edit (proved the plan's claim, not just asserted it), then recaptured via `run_black_box_fixture.py` — exits `0` again.
- [x] **Step 3 — Validators + cjs test**: New standalone `scripts/validate_security_audit_vendor.py` (frontmatter shape, no upstream broad-match phrases, house sections, opt-in mentions, four-control mentions, `needs_validation`, NOTICE/LICENSE/companion-SKILL.md attribution checks). New `tests/test_security_audit_vendor_cjs.py` — resolves `node` via `shutil.which`, **fails** (never skips) on missing/too-old Node, execs `node --test` on both vendored `.test.cjs` files. Two fixtures: `vendor-structure-check/`, `cjs-tests-pass/`.
- [x] **Step 4 — GATE**: Ran both fixtures via `scripts/run_black_box_fixture.py`, captured `skills/security-audit/black-box-run.json` (`cjs-tests-pass` is the canonical captured record — the stronger real-execution proof). `python3 scripts/check_skill_live.py security-audit` → exit `0`. Did not proceed past this point until green, per plan.
- [x] **Step 5 — INDEX/AGENTS/rules/exporter**: New `### security-audit` entry in `skills/INDEX.md` (+ hub-version bump `0.11.0` → `0.12.0`, SHOULD_FIX-optional). New row in `AGENTS.md` §4. New trigger line in all five rule files (`.cursorrules`, `.clinerules`, `.kilocoderules`, `.openhands_instructions`, `.cursor/rules/agent-bootstrap.mdc`) + `AGENTS.md` footer version bump. New `SkillConfig` entry in `scripts/export_codex_skills.py` with `quick_start` restating guidance-default + four-control check + not-a-substitute (SHOULD_FIX).
- [x] **Step 6 — install-grok.sh fix + dual-home test**: Confirmed the plan's claim live — the restore loop's `[[ -f "$ref" ]]` check silently skipped `references/cloudflare/` (a directory). Fixed the loop to walk `find "${src_ref}" -type f -print0` recursively, excluding `source.md` at any depth. New `tests/test_install_grok_nested_references.py` (2 tests: exporter's already-recursive preserve locked in via a synthetic skill + `patch.dict(SKILL_CONFIGS, ...)`; install-grok's now-fixed restore proved against the real vendored `security-audit` tree, written only to an isolated tmp target). Ran `install-grok.sh --local --force` for real against this checkout to populate `.grok/skills/security-audit/` — this also picked up **pre-existing, unrelated drift**: several skills' canonical `references/*` files (`interrogate`, `reflect`, `how`, `why`, `reply-contract`, `create-verification-skill`, `show-me-your-work`) had never been synced into `.grok/skills/` at all (a longstanding stale-sync gap, exposed by running `--force` for the first time in a while, not caused by this PR's own edits) — included as incidental cleanup, called out explicitly rather than silently bundled. Restored two accidentally `rm -rf`'d `.grok/agents/*.md` files caused by a **pre-existing, separate** self-referential-copy bug in `install-grok.sh`'s agent-install step (triggered only when `--target` equals the repo root) — left that bug itself unfixed, out of this plan's scope; documented as a leftover.
- [x] **Step 7 — security-reviewer/AGENTS rewrite**: Searched `agents/*.md` for the plan's cited "standalone security audits" sentence — it does **not** exist in `agents/security-reviewer.md` on current `main` (no `agents/*.md` file has a "When to Activate" section at all; that pattern only exists in `AGENTS.md` §3's mirrored role summaries). Rewrote the one real location, `AGENTS.md` §3, to state the persona never implicitly launches the six-phase workflow. Added a corroborating pointer in `agents/security-reviewer.md`'s existing "Related Skills" section for defense-in-depth (evidence-based addition, not fabricating a section the file never had).
- [x] **Step 8 — full suite**: `python3 -m unittest discover -s tests` — 133/133 pass (123 baseline + 10 new: 4 in `test_security_audit_vendor_cjs`, 4 in `test_pstack_skills` companion/CLI checks, 2 in `test_install_grok_nested_references`). `tests/test_index_live_binding.py` green with `security-audit` listed and live. `node --test` on both vendored `.test.cjs`: 65/65 pass.

**Explicitly did NOT do** (hard constraints): did not add `security-audit` (or `blast-radius`) to `GRANDFATHERED_SKILLS` in `scripts/index_skills.py` (grepped to confirm). Did not run a full six-phase security audit against `agent-bootstrap` itself. Did not open, read, or reference GMA-54. Did not self-merge; PR opened as draft.

**Verification**: `python3 -m unittest discover -s tests` — 133/133 pass. `python3 scripts/check_skill_live.py security-audit` → 0. `python3 scripts/check_skill_live.py blast-radius` → 0. `python3 scripts/index_skills.py` → "Every non-grandfathered INDEX.md entry is live-eligible." `node --test` on `validate-findings.test.cjs` + `validate-coverage-ledger.test.cjs` → 65/65 pass.

**Next**: Blair reviews the draft implement PR; do not self-merge.
...
**What Was Done**
- [x] Fixed 9 off-by-one section cross-references in the plan doc itself (several `§8` refs actually meant `§7`/`§9`/`§10`/`§14` — a should-fix named directly by the task). Did **not** reopen the plan-track otherwise; `tests/test_hoh_plan_wiki_citation.py` (§2/§15, unrelated) re-confirmed green before and after.
- [x] `scripts/validate_evidence_packet.py` (new): the `E_t.json` schema validator — `head_sha` required (GB-4), `qa_status`/every record's `status` restricted to `verified | gap` only (GB-1/GB-6, rejects `partial`/`blocked`/"looks good" at both levels, crossed-pair aware — a bad packet-level status and a bad record-level status can both be flagged in the same run, in either direction), non-empty `execution_records` typed `screenshot | runtime_trace | fixture` (GB-1 "empty is a gap, not a pass"), `planner_handoff.update_targets`/`preservation_constraints` structural non-emptiness rules (GB-3), forbidden living-PII scan across every string value (bare `Lisa`/`Tanya` names, phone numbers outside the reserved `+1555XXXXXXX` fictitious range). `--expect-head-sha <sha>` implements the freeze-mismatch check; `--retry-then-escalate <a> <b>` implements GB-6 (one retry, then `ESCALATE` printed to stdout, **exit code 1** — never `2`, which stays reserved for `scripts/run_black_box_fixture.py`'s own environment-blocked verdict; documented explicitly in the new `SCHEMA.md` so escalate and blocked are never conflated).
- [x] `scripts/validate_preservation_gate.py` (new): GB-2's exact `## Preservation Gate` heading check (missing heading, or present-but-no-bullets, both flagged).
- [x] `scripts/check_planner_reads_et.py` (new): H-1's path-convention check (plan template must name the exact `evidence/E_t.json` path).
- [x] `scripts/check_evidence_index_is_progressive.py` (new): H-5's check (no raw `E_t` field dumps, must point at `evidence/E_<n>.json`/`evidence/E_t.json`).
- [x] **TDD, red-then-green confirmed for every script** before it existed (all four modules raised `ModuleNotFoundError` on first test run; implemented; reran green) — see this branch's git history for the literal before/after test output.
- [x] Nine named black-box fixtures under `skills/evidence-packet-protocol/fixtures/<case>/` (eight) and `skills/preservation-gate/fixtures/dt-missing-preservation-gate/` (one), each `case.json` + `README.md` + real sample data, **no `<placeholder>` argv anywhere** (mechanically checked by `tests/test_evidence_packet_protocol_fixtures.py::test_fixture_argv_contains_no_angle_bracket_placeholders`). Blair-pass-2-style should-fixes folded in directly:
  - `et-status-not-verified-or-gap`: **two** sample packets (`E_t.partial-blocked.json` qa_status `partial` + one record `blocked`; `E_t.blocked-partial.json` qa_status `blocked` + one record `partial`), one command validates both, `expected.stdout_contains` asserts all four literal tokens (`invalid qa_status: partial`, `invalid record status: blocked`, `invalid qa_status: blocked`, `invalid record status: partial`).
  - `freeze-sha-mismatch`: literal 40-char hex SHAs (`aaaa...a` packet `head_sha`, `--expect-head-sha bbbb...b`), not a `<candidate-sha>` placeholder.
  - `schema-retry-then-escalate`: exit code **1** (not the plan's original `2`), stdout contains `ESCALATE`; both `invalid-1.json`/`invalid-2.json` are genuinely schema-invalid.
  - Every one of the 9 fixture `README.md` files, plus both new `SKILL.md` Common Mistakes tables, names the `attack-12-fixture-io` REPEAT-closure class explicitly (argv/exit/stdout, red-then-green, never a comment) — checked mechanically by `test_every_fixture_readme_names_the_attack_12_repeat_closure_class`.
- [x] `tests/test_evidence_packet_protocol_fixtures.py` (new) IS the `attack-12-fixture-io` mechanical check: reuses `scripts/run_black_box_fixture.py`'s own `load_case`/`run_case` (the real system under test, not a reimplementation) to actually execute every fixture's literal argv and assert a real `"pass"` verdict — not a description, not a mock.
- [x] `skills/evidence-packet-protocol/SKILL.md` + `SCHEMA.md` (new, `1.0.0`): full GB-1/GB-3/GB-4/GB-6 rules + H-1 (path convention)/H-2 (role write-permissions, quoting the paper's own Planner/QA-Tester contract lines)/H-3 (warm-start contract, direct paper quote)/H-4 (per-role commit convention)/H-5 (progressive-index pointer). Includes a "Leftovers" section (mirroring the plan's §14) naming: no HoH runtime, Hardproof not installed, native-compiled-black-box gap, "Eleanor" `reply-contract` `1.3.0` overlay not reconciled, no Grok Bot CoS Lane/Kit overlay wiring in the hub.
- [x] `skills/preservation-gate/SKILL.md` (new, `1.0.0`): the exact heading, its comparison table vs. REPEAT (positive/never-closes vs. negative/mechanically-closed), and the "Placement decision" (own skill, not folded into `reply-contract` — `reply-contract` explicitly untouched, and the "Eleanor `reply-contract` `1.3.0`" naming ambiguity is called out so neither lineage is mistaken for the other).
- [x] `skills/multi-harness-coordination/SKILL.md` (`1.0.0` → `1.1.0`): **one** new "Optional: Preservation Gate field" subsection, immediately after the existing four-field-envelope subsection — a pointer only (no mechanism copy), no rewrite of anything else in the skill. Confirmed via `git diff` that the four-field-envelope content itself (`9259d42`'s already-landed steal) is byte-identical, not restolen. `skills/reply-contract/SKILL.md`, `skills/plan-code-review-workflow/SKILL.md`, `skills/expert-pr-review/SKILL.md` are all untouched (confirmed empty `git diff` on each); no `skills/adversarial-review/` directory created anywhere.
- [x] **Gate ordering respected**: captured real `skills/evidence-packet-protocol/black-box-run.json` and `skills/preservation-gate/black-box-run.json` via `scripts/run_black_box_fixture.py` — genuine `"verdict": "pass"`, real `skill_sha256` — **before** either skill was added to `skills/INDEX.md`. `scripts/check_skill_live.py <name>` exits `0` for both. **Directly observed the silent-refine guard fire on this session's own new skill**: after adding the Leftovers section to `evidence-packet-protocol/SKILL.md` post-capture, `check_skill_live.py evidence-packet-protocol` reported `stale` until recaptured, then `live-eligible` again — the exact mechanism this hub built in PR #11 working as designed on brand-new content, not just its original targets.
- [x] `scripts/index_skills.py`: confirmed "Every non-grandfathered INDEX.md entry is live-eligible" both new skills included, **neither** added to `GRANDFATHERED_SKILLS` (that allowlist is closed to the original 20 pre-gate skills only, per its own pinned-equality test).
- [x] `scripts/export_codex_skills.py`: two new `SkillConfig` entries (`evidence-packet-protocol`, `preservation-gate`); `tests/test_export_codex_skills.py` gained matching `assertIn` lines. Re-exported `.grok/skills/` via `--force` twice (once per `SKILL.md` edit) — confirmed each time that only the expected `references/source.md` file(s) changed and `grill-with-docs`'s hand-added reference files (`adr-format.md`, `context-format.md`) survived intact (the known, still-unfixed exporter gap from prior sessions did not recur here because nothing touched that skill).
- [x] `skills/INDEX.md`: two new entries added **after** the live-gate pass above, hub version `0.9.3` → `0.10.0`. `AGENTS.md` §4 "Other Key Skills" table gained two rows; footer version bumped to match. All five per-harness trigger files (`.clinerules`, `.kilocoderules`, `.cursorrules`, `.openhands_instructions`, `.cursor/rules/agent-bootstrap.mdc`) gained matching trigger lines, format-matched to each file's own existing convention (bullet-arrow with computed column-aligned padding for the three bullet-list files; table row for the other two).
- [x] Did **not**: clone or install `Flesymeb/HarnessOfHarness`, a "Hardproof" plugin, OpenCode, or any HoH runtime/scheduler; reopen GMA-9/GMA-14; start GMA-8/GMA-13/AWS; put any real living person's name or real phone number anywhere (only the intentional `Lisa`/`Tanya` forbidden-fixture pair and the reserved `+1555XXXXXXX` fictitious range appear in any sample — confirmed via `grep`); mint a teammate entity page for Kit or Lane; put a Grok Bot agent UUID in any hub skill; restole the `9259d42` four-field envelope; touch `skills/reply-contract/SKILL.md`, `skills/plan-code-review-workflow/SKILL.md`, or `skills/expert-pr-review/SKILL.md`; create `skills/adversarial-review/`.

**Verification Evidence**
- `python3 -m unittest discover -s tests` from `/workspace`: **96/96 pass** (45 pre-existing + 51 new: 34 in `test_validate_evidence_packet.py`, 5 in `test_validate_preservation_gate.py`, 4 in `test_check_planner_reads_et.py`, 4 in `test_check_evidence_index_is_progressive.py`, 4 in `test_evidence_packet_protocol_fixtures.py`), confirmed at every commit on this branch.
- Every one of the four new validator scripts was confirmed red (`ModuleNotFoundError`) before it existed and green immediately after, run standalone.
- `python3 -m unittest tests.test_evidence_packet_protocol_fixtures -v`: 4/4 pass, including a real subprocess execution of all 9 fixtures' literal argv via `scripts/run_black_box_fixture.py`'s own `run_case` — genuine `"pass"` verdicts, not descriptions.
- `python3 scripts/check_skill_live.py evidence-packet-protocol` / `preservation-gate`: both exit `0`, `"live-eligible"`.
- `python3 scripts/index_skills.py`: `"Every non-grandfathered INDEX.md entry is live-eligible."`
- `git diff --stat -- .grok/skills/grill-with-docs/` after each re-export: empty (no recurrence of the known, unrelated exporter reference-file gap).
- `git diff | grep -iE "blair"`: no matches in any shipped skill/test/fixture content (reviewer name stays in `memory-bank/` prose only, per this hub's established convention).
- Branch: `cursor/implement-hoh-schema-steal-4b7c`. New draft PR against `main`; not merged, not self-reviewed (per task instructions — Blair impl-reviews, then Kit black-box-tests; reading the PR is not a pass for Kit either).

---

## 2026-09-02 — Task "hoh-schema-steal" revision: Blair grok-4.6 REVISE, 3 must-fix items closed

**Task**: Blair grok-4.6's REVISE on [PR #15](https://github.com/gman-robotics/agent-bootstrap/pull/15) at commit `d37a75e4`. Same branch `cursor/hoh-schema-steal-plan-aacd`, same PR, plan-only (no `evidence-packet-protocol`/`preservation-gate` skill files, no `Et` validator as a product). Three must-fix items:

**1. REPEAT class: wiki citation (`wiki-citation` / `hoh-plan-wiki-sot`)**
- §2 originally cited `https://github.com/Flesymeb/HarnessOfHarness/wiki/harness-of-harness` (the upstream HoH product repo's own GitHub wiki) as the fetched wiki source of truth. Required SoT per the review: `ThomasGinter/tmg-wiki`, `entities/harness-of-harness.md` @ `0651e608fded1c0676951ff16555b97e2671710d` (wiki PR #99).
- Attempted to fetch the required file directly: `gh api /repos/ThomasGinter/tmg-wiki`, `git ls-remote https://github.com/ThomasGinter/tmg-wiki(.wiki).git`, and a direct `raw.githubusercontent.com`/`github.com` blob fetch all returned "not found" (private repo, no owner credentials available to this run). Confirmed `ThomasGinter` is a real GitHub user (9 public repos, none named `tmg-wiki`) via `gh api /users/ThomasGinter` and `/users/ThomasGinter/repos` — consistent with `tmg-wiki` being a private personal repo outside this run's access. Rewrote §2 to cite the required repo/path/commit **by identity**, with an explicit, honest note that this run's tools lack read access and that the plan's substantive facts (Table 3, Listing 1) are independently cross-checked against the primary source (arXiv `2609.01481`) directly, not solely relayed through the unreachable wiki page — no wiki content was fabricated.
- Per `skills/triage-review-feedback/SKILL.md` Step 3, an instance fix to §2's prose alone does not close a REPEAT-tagged item — added the required mechanical check: `tests/test_hoh_plan_wiki_citation.py` (new, 5 tests). `find_wiki_citation_violations(text)` checks for the three required literals (`ThomasGinter/tmg-wiki`, `entities/harness-of-harness.md`, `0651e608fded1c0676951ff16555b97e2671710d`) and the absence of the forbidden `Flesymeb/HarnessOfHarness/wiki` substring.
- **Self-caught bug during this same revision**: a first draft scoped the forbidden-substring check to the *entire* document, which self-tripped against §15's own historical description of the bug (describing what was wrong necessarily requires naming the bad URL once). Fixed by scoping `find_wiki_citation_violations` to the `## 2. Sources` section only (`extract_wiki_sources_section`, regex-bounded to the next `## ` heading) — this is also the more faithful reading of the review (the bug is what §2 *cites as SoT*, not whether the string appears anywhere in the file for any reason, including a later prose discussion of the fix itself).
- **Red-then-green confirmed both ways**: (a) synthetic fixture `OLD_BUGGY_SECTION_2_FIXTURE` reproducing the exact old bug trips the checker (`test_old_fixture_reproduces_the_bug_and_is_caught`), including a "shallow fix" variant that removes only the bad URL without adding the real citation (`test_a_fixed_but_incomplete_citation_is_still_caught`); (b) **live proof against the real file**: temporarily mutated the real, committed `hoh-schema-steal-plan.md`'s §2 back to the exact old bug, ran the suite — `FAILED (failures=2)` on `test_real_plan_document_cites_the_required_wiki_sot` and `test_real_plan_document_may_discuss_the_old_bug_outside_the_sources_section` — then restored via `cp` from a pre-edit backup, confirmed byte-identical via `diff` (empty), and re-ran — `Ran 5 tests ... OK`. Both the mutation and the restore were done for real, not simulated.

**2. NEW: `Et` `qa_status` enum allowed `partial`/`blocked`**
- The plan's fenced `E_t` JSON schema (§7) had `"qa_status": "partial"` with a comment enum of `"verified" | "partial" | "blocked"`, contradicting the plan's own prose one paragraph below it ("verified | gap only"). Fixed the fenced example to `"qa_status": "gap"` with enum `"verified" | "gap"`; tightened the prose to explicitly reject `partial`/`blocked`/`"looks good"` at **both** the packet root and every record. Added a clarifying footnote in §2 immediately after the Listing 1 quote, noting the paper's own example uses `"partial"` but the house schema does not carry that value forward — so the historical quote and the house lock cannot be conflated.
- Updated the `et-status-not-verified-or-gap` fixture row: `expected.stdout_contains` now names both `"invalid qa_status: partial"` and `"invalid record status: blocked"`, and added a note that the fixture's sample packet must carry both violations simultaneously (one bad root field, one bad record field) so a single validator run is checkable for both, per the review's "reject BOTH packet-level and record-level" requirement.

**3. NEW: leftover optional-vs-required `head_sha` question beside the spec-gate**
- §6's GB-4 row said "add optional `head_sha` field" while §7's schema comment said "required" — an unresolved contradiction functioning as an open question sitting next to the closing Approve/Reject card, which the reply-contract convention forbids. Resolved: `head_sha` is **required**, not optional, everywhere it's mentioned (§6 GB-4 row, §7 schema comment, §9 prose, §11 done-when) — an optional freeze field would make GB-4's whole point (frozen candidate identity) theater, which is exactly the risk already named in §13. Re-presented the same spec-gate card at the end of the document with no leftover question beside it.

**Verification Evidence**
- `python3 -m unittest tests.test_hoh_plan_wiki_citation -v`: 5/5 pass on the final, fixed real file.
- Live mutate-then-restore proof on the real committed plan doc (not a temp copy): red (`FAILED (failures=2)`) on the reintroduced old bug, byte-identical restore confirmed via `diff`, green (`OK`) after restore — reproduced twice during this revision (once before the section-scoping fix, once after, to prove both the original whole-document check and the corrected section-scoped check actually discriminate old-vs-fixed on the real file).
- `python3 -m unittest discover -s tests`: **45/45 pass** (40 pre-existing + 5 new), run clean at the end of the revision.
- `git diff --cached --check`: clean. `git diff --cached | grep` for forbidden roster/model-pair names (excluding the task-directed Blair/CoS/Kit/Lane/model-name references, which this task explicitly requires naming): no unexpected matches.
- Branch: `cursor/hoh-schema-steal-plan-aacd` (unchanged). PR: same PR, updated in place — [#15](https://github.com/gman-robotics/agent-bootstrap/pull/15). No second PR opened. Not merged. Still plan-only: `docs/projects/agent-bootstrap/hoh-schema-steal-plan.md` (revised) + `tests/test_hoh_plan_wiki_citation.py` (new, documentation mechanical lock only) + this memory-bank update — no `skills/evidence-packet-protocol/`, `skills/preservation-gate/`, or any validator script was created.

---

## 2026-09-02 — Task "hoh-schema-steal": plan-only document, no implementation

**Task**: Tom's 2026-09-02 decision — steal Harness-of-Harness's (HoH, [`Flesymeb/HarnessOfHarness`](https://github.com/Flesymeb/HarnessOfHarness), arXiv `2609.01481`) evidence-packet (`E_t`) schema and Preservation Gate concept onto both Grok Bot and Hermes, schema/process only — not the HoH product, not autonomous no-human development. Explicit instruction: plan only, no skill/schema/validator/fixture as live code; author model claude-sonnet-5; next reviewer Blair grok-4.6; no implement-track work until CoS records a literal Blair Approve.

**What Was Done**
- [x] Fetched and cited, read-only (not cloned): the HoH wiki/README overview (`https://github.com/Flesymeb/HarnessOfHarness/wiki/harness-of-harness`, resolves to the repo's README-equivalent overview at fetch time) and the arXiv paper `2609.01481` abs page + PDF text. Verified the task-supplied locked ablation numbers (Table 3: Full HoH@3 `71.52`; w/o Plan Update `−8.13`; w/o Evidence Feedback `−6.28`; w/o Warm-Start `−7.85`) and Listing 1's normalized `E_t` JSON structure directly against the fetched paper text — matched exactly, not recomputed, not invented.
- [x] Wrote exactly one new file: `docs/projects/agent-bootstrap/hoh-schema-steal-plan.md`, following the existing `docs/projects/agent-bootstrap/mem0-lessons-plan.md` precedent (a plan-shaped doc living directly under `docs/projects/agent-bootstrap/`, outside the four standard docs-protocol types) for style and section shape.
- [x] Plan maps every named item to a concrete implement-track target: GB-1 (claim-bound evidence packet, typed `execution_records`, `verified|gap` status only) → new `skills/evidence-packet-protocol/SKILL.md` + `SCHEMA.md`; GB-2 (Preservation Gate field, distinct from REPEAT) → new `skills/preservation-gate/SKILL.md`, with the exact distinction table (positive/carried-forward vs. negative/mechanically-closed) spelled out once; GB-3 (repair gaps + deliver one new capability) → a rule inside GB-1's skill, no new file; GB-4 (frozen candidate identity) → `head_sha` field added to the `E_t` schema root, extending the existing `scripts/check_skill_live.py`/`skill_sha256` pattern rather than installing anything branded "Hardproof" (rhyme only, explicitly not installed); GB-5 (Lane screenshots inside `E_t`) → a consequence of the GB-1 schema shape, zero new surface; GB-6 (schema-or-retry-once then escalate) → a rule inside GB-1's skill, reusing the existing `"blocked"`/exit-`2` verdict convention from `skills/black-box-agent-qa/SCHEMA.md` rather than inventing a fourth verdict; H-1 (same `E_t` on disk, next planner must read it) → `evidence/E_t.json` path convention, living next to the *product* repo/branch, not inside this hub; H-2 (read-only planner/tester worktree, Developer sole writer, keep model pairs distinct) → a role-permissions rule citing the paper's own Planner/QA Tester "do not modify production code" lines; H-3 (warm-start developer contract) → a rule quoting the paper's own Developer contract line, directly the mechanism the w/o Warm-Start ablation measures; H-4 (per-role git commit) → a rule extending the existing `multi-harness-coordination` Orchestrator Checklist pattern; H-5 (progressive-disclosure index, not a full dump) → `evidence/INDEX.md` per product repo, modeled on this hub's own `skills/INDEX.md` convention.
- [x] Named 9 black-box fixtures with literal `input.command` argv, `expected.exit_code`, and `expected.stdout_contains`, per `skills/black-box-agent-qa/SCHEMA.md`'s existing contract (unchanged runner, `scripts/run_black_box_fixture.py`): `et-schema-valid`, `et-status-not-verified-or-gap`, `et-missing-execution-record`, `et-living-pii`, `dt-missing-preservation-gate`, `freeze-sha-mismatch`, `schema-retry-then-escalate`, `next-planner-reads-et` (H-1), `evidence-index-not-full-dump` (H-5). None of the underlying validator scripts (`scripts/validate_evidence_packet.py`, `scripts/validate_preservation_gate.py`, etc.) were written — named as implement-track targets only.
- [x] **Placement decision, with reason, for the Preservation Gate**: canonical definition in a new small skill (`skills/preservation-gate/SKILL.md`), not a `skills/reply-contract/SKILL.md` patch — the gate is a field on the plan artifact (`Dt`) itself, consumed by the Developer role, not part of `reply-contract`'s human-stamp mechanism. Only a one-line pointer (no mechanism copy) is proposed for `skills/multi-harness-coordination/SKILL.md`, next to its existing four-field-envelope subsection. `skills/reply-contract/SKILL.md` (this hub's copy at `1.4.0`) is explicitly **not touched**, which also sidesteps the "Eleanor reply-contract `1.3.0` vs. this hub's `1.4.0`" ambiguity named in the task instructions — flagged as a leftover for a future overlay-vs-hub reconciliation task, not resolved here.
- [x] Explicitly did **not**: clone `Flesymeb/HarnessOfHarness`; touch `arm`; start AWS or reopen GMA-8/13/14; mint a Kit/Lane entity page (referenced both only as already-existing Scout-scoreboard roles); rewrite `skills/plan-code-review-workflow/SKILL.md` or `skills/expert-pr-review/SKILL.md` (hub lock, confirmed untouched via `git diff --stat`); restale swarm-forge cockpit/envelope (confirmed already at `9259d42`, not reopened); add `skills/adversarial-review/` to the hub tree (named in the task as Grok Bot local-only, not restolen here); ship any live `SCHEMA.md`, `SKILL.md`, `case.json`, or validator script — every JSON/Markdown example in the plan is a fenced spec block inside the one document, not a separate committed file.
- [x] Named risks (schema theater without freeze, CoS overlay drift from hub, PII bus, same-model temptation, overbuild into a runtime) and leftovers (Hardproof not installed, native-compiled-black-box gap, HoH-lite unpublished, GMA-14 fog, Eleanor reply-contract `1.3.0`) explicitly in the plan document, per the task's required section list.

**Verification Evidence**
- `git diff --cached --check` from `/workspace`: clean.
- `git diff --cached --stat`: exactly one file, `docs/projects/agent-bootstrap/hoh-schema-steal-plan.md` (266 insertions), plus this memory-bank update — no `skills/`, `scripts/`, or `.grok/` file touched.
- Locked ablation numbers and Listing 1 structure cross-checked line-by-line against the fetched arXiv `2609.01481` text (`grep` for `"Table 3"`, `"71.52"`, `"8.13"`, `"6.28"`, `"7.85"`, and the Listing 1 JSON fields) — exact match, confirmed before writing the plan's citation section.
- Branch: `cursor/hoh-schema-steal-plan-aacd`, off `main` at `b0838a2`. PR: opened as one draft PR against `main`. Not merged, not self-reviewed — Blair grok-4.6's grill is the next step, gated on a literal recorded Approve before any implement-track PR starts.
...
## 2026-08-26 — Follow-up PR: heading-negation leak in show-me's photon-safe scanner

**Task**: [PR #13](https://github.com/gman-robotics/agent-bootstrap/pull/13) squash-merged to `main` as `3294849` (native `show-me` skill + photon-safe scanner). A pass-2 adversarial review of the merged state found one REPEAT must-fix (same failure class as PR #13's own pass-1 blocker, "reading the skill Markdown is not a pass"): `find_photon_safe_violations()` in `tests/test_show_me_skill.py` joined a markdown unit's *nearest heading* into the same negation-context string as the unit itself (`f"{heading} {unit}".lower()`), so a heading's own words could cancel a violation in a completely different, unnegated body line beneath it. New branch (`cursor/show-me-heading-negation-leak-b2ea`), new draft PR against `main` — #13 not reopened, not reverted, not merged again, no second PR on the merged branch.

**Reproduced the leak live before touching production logic**: ran `find_photon_safe_violations` against two synthetic probes on the unfixed checker — `## Recipe: mermaid (opt-in only)` followed by a bare `Bash(open diagram.html)` body line, and the same body line under a `## Pitfalls` heading (whose own name contains the substring `"pitfall"`, one of `NEGATION_MARKERS`) — both returned `[]` (zero violations), confirming the exact gap named in the review.

**What Was Done**
- [x] **Must-fix**: `find_photon_safe_violations` now scopes negation to the unit's own text only (`negation_context = unit.lower()`), never the heading. The heading is still threaded through to the violation message for readability, but plays no role in deciding whether a match counts as a violation.
- [x] **TDD**: added `HEADING_NEGATION_LEAK_RECIPE_CASE` and `HEADING_NEGATION_LEAK_PITFALLS_CASE` fixtures plus two new permanent tests (`test_heading_words_do_not_negate_a_violating_body_line_under_an_opt_in_recipe_heading`, `test_heading_words_do_not_negate_a_violating_body_line_under_a_pitfalls_heading`) — confirmed **red** against the unfixed checker (both `AssertionError: [] is not true`) before editing `find_photon_safe_violations`, confirmed **green** immediately after.
- [x] **Real-file fallout, fixed correctly rather than papered over**: stripping the heading from the negation window immediately flipped `test_skill_never_recommends_opening_html_or_defaulting_to_mermaid_html` red against the real, committed `skills/show-me/SKILL.md` — Pitfalls item 4 (`` A `Bash(open ...html)`-style command to launch a generated visual ``) had relied entirely on the heading's `"pitfall"` substring to stay green; it carried no negation word of its own. Reworded to `` Never a `Bash(open ...html)`-style command to launch a generated visual `` — matching how every other prohibition in this file is already phrased (e.g. "Never write a command that opens...") — so it now carries its own per-unit negation marker honestly, not as a heading-borrowed loophole. `skills/show-me/SKILL.md` version bumped `1.0.0` → `1.0.1` (frontmatter + `tests/test_show_me_skill.py`'s version assertion + the fixture `README.md`'s check-1 description, all three updated together).
- [x] **Live re-verification, not just the synthetic fixtures**: temporarily reintroduced the exact old-fictional-case pair of bullets (mermaid/HTML default + `Bash(open diagram.html)`) into the real committed `skills/show-me/SKILL.md`, ran `python3 -m unittest tests.test_show_me_skill -v` — failed with 3 violations listed by name (`FAILED (failures=1)`) — then reverted via the pre-edit backup and re-ran to confirm clean green again. This is in addition to (not a replacement for) the two new permanent unit-test fixtures.
- [x] **Fixture README** (`skills/show-me/fixtures/reply-contract-link-check/README.md`): new "Revision (adversarial review pass 2, heading-negation leak...)" paragraph naming the exact bug, the two reproduced probes, and the Pitfalls item 4 real-file fallout; check 7's negation-scope wording corrected (no longer says "that unit or its heading"); new checks 9–10 documenting the two permanent regression fixtures; check 1's version string updated to `1.0.1`.
- [x] Re-exported `.grok/skills/show-me/` via `python3 scripts/export_codex_skills.py --output-dir .grok/skills --force` — confirmed only `.grok/skills/show-me/references/source.md` changed (byte-identical to the two real edits: version bump + Pitfalls item 4 wording), and `git diff --stat -- .grok/skills/grill-with-docs/` stayed empty (no recurrence of the old exporter reference-file gap).
- [x] Recaptured `skills/show-me/black-box-run.json` via `scripts/run_black_box_fixture.py` **after** the checker was green on the real file and red on both new heading-leak fixtures (and on the pre-existing old-fictional-case fixture) — fresh `verdict: pass`, fresh `skill_sha256` matching the edited `SKILL.md`. Directly observed the staleness guard fire mid-session: `python3 scripts/check_skill_live.py show-me` reported `not live: run record is stale` immediately after editing `SKILL.md`, then `live-eligible` immediately after recapture.
- [x] Confirmed `show-me` still **not** in `scripts.index_skills.GRANDFATHERED_SKILLS` (`'show-me' in GRANDFATHERED_SKILLS` → `False`) and `python3 scripts/index_skills.py` still reports "Every non-grandfathered INDEX.md entry is live-eligible."
- [x] Bumped hub version footers (`AGENTS.md`, `skills/INDEX.md`) `0.9.2` → `0.9.3` to match the `show-me` skill version bump, per this hub's established convention.
- [x] **Self-caught convention slip**: an early draft of the new test docstrings and fixture README named the human reviewer ("Blair pass 2") directly in committed test/skill source — confirmed via `git show origin/main:tests/test_show_me_skill.py | grep -i blair` (no prior occurrence anywhere in committed skill/test/fixture *content*, only ever in commit messages and `memory-bank/` prose) that this would be a new convention break, not a repeat of an existing pattern. Reworded all four occurrences to generic "adversarial review pass 2" before committing; re-ran the forbidden-name grep clean.
- [x] Did **not**: touch `EstateGuruRepo`, `genealogy-wiki`, or `arm` (not present in or referenced by this checkout); add `show-me` to `GRANDFATHERED_SKILLS`; revert or reopen PR #13; open a second PR on the merged branch; merge anything; self-review (per task instructions — author model is claude-sonnet-5, human/adversarial reviewer owns the review).
- **Should-fix, explicitly left as leftovers (not cheap, not gold-plated)**: (1) "leftover-vs-bug I/O fixture tightening" — `reply-contract`'s own "Leftover vs bug" wording (`## Show-me table` pitfall list, item 5) has no dedicated black-box fixture of its own yet; scoping and building one is a fixture-design task in its own right, not a one-line fix, so left for a future pass. (2) "HTML-row-with-no-recipe case" — `reply-contract`'s "Pick one visual" table has a `one HTML file` row with no corresponding `## Recipe: HTML` section in `show-me` (only `mermaid (opt-in only)` covers the non-text-fence case); fixing this cleanly means either adding an HTML recipe (scope creep on a skill whose entire point is "never build/open HTML by default") or editing the table (touches `reply-contract`, out of this task's SKILL.md-scoped fix), so also left as a named leftover rather than a rushed edit.

**Verification Evidence**
- `python3 -m unittest tests.test_show_me_skill -v` (unfixed checker): 2 new fixtures failed with `AssertionError: [] is not true`, both explicitly asserting the leak (red).
- `python3 -m unittest tests.test_show_me_skill -v` (fixed checker, before the real-file wording fix): 1 failure — `test_skill_never_recommends_opening_html_or_defaulting_to_mermaid_html`, 2 violations both naming Pitfalls item 4 (red, confirming the real-file fallout was caught, not missed).
- `python3 -m unittest tests.test_show_me_skill -v` (fixed checker + reworded Pitfalls item 4): **12/12 pass** (10 pre-existing + 2 new).
- `python3 -m unittest discover -s tests`: **40/40 pass** (38 pre-existing + 2 new).
- Live reintroduction of the old fictional violating pair into the real, committed `SKILL.md`: `FAILED (failures=1)`, 3 named violations; reverted via backup, re-ran clean (`OK`).
- `python3 scripts/check_skill_live.py show-me`: `not live: ... stale` immediately after the `SKILL.md` edit, `live-eligible: ...` immediately after `scripts/run_black_box_fixture.py` recapture.
- `python3 scripts/index_skills.py`: "Every non-grandfathered INDEX.md entry is live-eligible."
- `git diff --stat -- .grok/`: exactly `.grok/skills/show-me/references/source.md` (2 lines changed, matching the two real edits); `git diff --stat -- .grok/skills/grill-with-docs/` empty.
- `git diff | grep -iE "reed|blair|\bkit\b|\bcos\b|wren|grok bot|claude-sonnet-5|grok-4\.6|prime agent|ipython rlm|\bdune\b"`: no matches (after the self-caught "Blair" reword, above).
- `git diff --check`: clean.
- Branch: `cursor/show-me-heading-negation-leak-b2ea`. New draft PR opened against `main`; PR #13 not reopened, not reverted, not merged again.
...
## 2026-08-26 — PR #13 revision: adversarial review found the fixture was still a heading-grep, and a dual-tree leftover of the fictional pairing line

**Task**: Adversarial review of [PR #13](https://github.com/gman-robotics/agent-bootstrap/pull/13) at `b06acbd` (COMMENT verdict "revise", GitHub review id `5032697308`) found two blockers, both treated as must-fix per the task instructions (the second was originally called should-fix, then explicitly promoted to must-fix in a follow-up message):
1. **REPEAT** — same failure class as PR #11 blocker #3 ("reading the skill Markdown is not a pass"): `skills/show-me/fixtures/reply-contract-link-check/`'s captured "live" evidence (`tests.test_show_me_skill`) only ever `read_text()`'d Markdown and asserted headings/paths/frontmatter tokens. A `SKILL.md` that recommended `Bash(open file.html)` or made mermaid/HTML the default visual would still have passed every one of those 7 tests — no leftover-vs-bug I/O, no photon-safe / one-visual / no-`Bash(open)` case.
2. **NEW** — `scripts/export_codex_skills.py`'s `reply-contract` `SkillConfig.quick_start` (line ~345) still read the old pathless "Pair with show-me: one tree, stack, or diff" after the canonical `skills/reply-contract/SKILL.md` had already been fixed to load the real path. Grok/Codex users load the generated `.grok/skills/reply-contract/SKILL.md` first, which is built from this exact `quick_start` tuple, not from the canonical source — so the dual-tree wrapper still shipped the fiction this PR exists to kill.

Same branch/PR (`cursor/show-me-native-skill-6d8c` / #13), not merged, not self-reviewed, no second PR opened.

**What Was Done**
- [x] **Must-fix 1 (REPEAT close)**: added a real content scanner, `find_photon_safe_violations()`, directly to `tests/test_show_me_skill.py` (the module the fixture's `case.json` already runs — no new fixture needed, same one extended per the task instruction). It splits `SKILL.md` into markdown units (one bullet/paragraph, tagged with its nearest `##` heading) — a first attempt using a flat character-radius window was tried and found to leak a neighboring bullet's negation word ("never"/"no ") across bullet boundaries, masking a real violation; the unit-scoped redesign fixes that. A unit is flagged only if it matches a forbidden-guidance pattern (recommends opening HTML/a browser, recommends a `Bash(open ...)` command, or treats mermaid/HTML as the *default* visual) with no negation marker (`never`, `no `, `without`, `opt-in`, `pitfall`, `only if`, `unless`, ...) in that unit or its heading.
- [x] New test `test_skill_never_recommends_opening_html_or_defaulting_to_mermaid_html` runs the scanner against the real, committed `skills/show-me/SKILL.md` and requires zero violations (currently true, unmodified `SKILL.md`).
- [x] New test `test_find_photon_safe_violations_catches_the_old_fictional_case` runs the same scanner against a synthetic `OLD_FICTIONAL_CASE_SKILL_TEXT` (a bullet list stating a mermaid/HTML default plus a `Bash(open diagram.html)` command) and requires it to return violations — proves the checker is not vacuous, permanently baked into the suite.
- [x] **Proved the check goes red on the real file, not just synthetic text**: temporarily replaced the real "opt-in only ... never by default" bullet in `skills/show-me/SKILL.md` with the exact violating pair of bullets (mermaid/HTML default + `Bash(open diagram.html)`), ran `python3 -m unittest tests.test_show_me_skill -v` — result: `FAILED (failures=1)`, exit code `1`, on `test_skill_never_recommends_opening_html_or_defaulting_to_mermaid_html`, reporting all 3 violations with their exact text. Reverted the file immediately after (`diff` against a pre-edit backup confirmed byte-identical restore); re-ran the suite — green again, `Ran 10 tests ... OK`.
- [x] **Must-fix 2 (NEW close)**: `scripts/export_codex_skills.py` `reply-contract` `quick_start` line 2 now reads "Load `skills/show-me/SKILL.md` for the one visual (tree, stack, or diff); never reimplement its recipes here. No mermaid/HTML on Photon unless asked." New test `tests/test_export_codex_skills.py::test_reply_contract_quick_start_points_at_the_real_show_me_path` asserts the real path is present and the old "Pair with show-me" phrase is gone — confirmed **red** against the unfixed exporter (`git stash` of just that file), confirmed **green** after.
- [x] Re-exported `.grok/skills/` via `python3 scripts/export_codex_skills.py --output-dir .grok/skills --force` — `git status --porcelain .grok/` shows only `.grok/skills/reply-contract/SKILL.md` changed (the quick_start line), no hand-added reference files dropped elsewhere.
- [x] Should-fix (named directly by the task, folded in): `test_show_me_is_exported_to_the_grok_dual_tree` now also asserts `.grok/skills/show-me/references/source.md` exists (previously only the thin `SKILL.md` wrapper was checked) and that it retains the recipe content with the `Last updated:` footer stripped (exporter design, unchanged). New `test_show_me_is_not_grandfathered_into_the_live_gate` asserts `"show-me" not in scripts.index_skills.GRANDFATHERED_SKILLS` (the optional should-fix item) — still true, `GRANDFATHERED_SKILLS` itself untouched.
- [x] `skills/show-me/fixtures/reply-contract-link-check/README.md` and `case.json` updated to describe the new checks 7–8 (content scan + non-vacuity proof) and the real-file red/green reproduction, matching the extended fixture.
- [x] Recaptured `skills/show-me/black-box-run.json` via `scripts/run_black_box_fixture.py` against the final content (real `stderr_tail`, fresh `skill_sha256` of the **unchanged** `SKILL.md` — the fixture/tests changed, the skill file itself did not). `python3 scripts/check_skill_live.py show-me` exits `0`.
- [x] Removed the specific external reviewer persona name from all shipped test/fixture/doc content (comments in `tests/test_show_me_skill.py`, `tests/test_export_codex_skills.py`, and the fixture `README.md` originally named it) after a forbidden-name grep flagged it — per this repo's established convention (see the 2026-08-22 swarm-forge session's `grep -iE "reed|blair|..."` check), reviewer/persona names belong in `memory-bank/` session notes, not in shipped skill/test/doc content. Re-confirmed `git diff | grep -iE "reed|blair|\bkit\b|\bcos\b|wren|grok bot|claude-sonnet-5|grok-4\.6|prime agent|ipython rlm|\bdune\b|estateguru|genealogy|\barm\b"` → no matches after the edit.
- [x] Did **not**: touch `skills/show-me/SKILL.md` or `skills/reply-contract/SKILL.md` content (confirmed via `git diff --stat` on both — empty; only the *exporter config* and *tests/fixture* needed a fix, not the canonical skill prose); add `show-me` to `GRANDFATHERED_SKILLS`; open a second PR; merge; self-review; touch `EstateGuruRepo`, `genealogy-wiki`, or `arm` (confirmed absent from this checkout via `find`).

**Verification Evidence**
- `python3 -m unittest discover -s tests`: **38/38 pass** (34 pre-existing + 4 new: 3 in `test_show_me_skill.py`, 1 in `test_export_codex_skills.py`).
- `python3 -m unittest tests.test_show_me_skill -v` against a temporarily-corrupted real `skills/show-me/SKILL.md` (old-fictional-case bullets injected in place of the real "opt-in only" bullet): **`FAILED (failures=1)`**, exit `1` — the exact red-on-old-case proof the review required. Restored, re-ran: green.
- `python3 scripts/run_black_box_fixture.py --fixture skills/show-me/fixtures/reply-contract-link-check --skill show-me --out skills/show-me/black-box-run.json`: real captured `{"verdict": "pass", ...}`.
- `python3 scripts/check_skill_live.py show-me`: exit `0`, "live-eligible... matches the current SKILL.md".
- `python3 scripts/index_skills.py`: "Every non-grandfathered INDEX.md entry is live-eligible."
- `git status --porcelain .grok/`: only `reply-contract/SKILL.md` modified after `--force` re-export.
- `git diff | grep -iE "reed|blair|\bkit\b|\bcos\b|wren|grok bot|claude-sonnet-5|grok-4\.6|prime agent|ipython rlm|\bdune\b|estateguru|genealogy|\barm\b"`: no matches.
- Branch unchanged: `cursor/show-me-native-skill-6d8c`. Same PR #13 updated in place, not merged, not self-reviewed, no second PR.
