...
- **Do not clone** `Flesymeb/HarnessOfHarness`. Nothing in §4–§6 requires the upstream code; HoH-lite is not yet published (see §14 Leftovers) and would not be vendored even if it were — this is a schema/process steal, not a code adoption.
...
| **GB-2** | Preservation Gate field on the plan markdown, distinct from REPEAT | `skills/preservation-gate/SKILL.md` (new, canonical) + one-line pointer in `skills/multi-harness-coordination/SKILL.md` | Field spec in §8; **not** a `reply-contract` patch (see §8 "Placement decision") |
...
| **GB-4** | Frozen candidate identity: Kit/Lane evidence bound to one git SHA; no edit rights on the candidate | `skills/evidence-packet-protocol/SCHEMA.md` — add a **required** `head_sha` field at the packet root (§7) | "Hardproof" rhyme only — **Hardproof itself is not installed** (see §14 Leftovers). `head_sha` is required, not optional — optional would make the freeze theater (§13 Risks) |
...
| **H-5** | Progressive-disclosure index, not a full dump | `evidence/INDEX.md` (per product repo, sibling to `evidence/E_t.json`) — modeled on this hub's own `skills/INDEX.md` progressive-disclosure convention | Fixture: `evidence-index-not-full-dump` (§10) |
...
3. **`skills/evidence-packet-protocol/fixtures/<case-name>/`** (new, one dir per fixture named in §10) — `case.json` + `README.md` each, same shape as `skills/black-box-agent-qa/fixtures/`.
4. **`skills/preservation-gate/SKILL.md`** (new, canonical definition) — the exact heading, required list shape, and the explicit distinction from REPEAT (§8).
